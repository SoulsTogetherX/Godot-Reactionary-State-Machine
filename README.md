...
**HSMMaster** is the root of any state machine. Within here, all calculations and stage changes are processed.
...
**HSMModule** are equivalent to **HSMNode** in implemented functionally. However, they cannot be the current state in a state machine, and they do not propagate functionality to their children.
...
The **HSMContext** node is a shared blackboard of information between states of the same or different state machines. It can either hold actions (bools) or values (Variants). Any connected state machine will receive a signal when an action is turned on, off, toggled, or when values are changed. You are also able to access the context object at any time in your **HSMNode** and **HSMModule** nodes via the `get_context()` method.
...
In the "Idle" and "Running", you obviously want to be able to transition to "Jumping" whenever needed. In a traditional state machine, you'd need to add a method to transition to "Jumping" in both Idle and Running whenever you press the "jump" button. The same for the "jump" state whenever you hit the ground. Here lies redundancy in conditional checking and handling.

This alone fine, but what happens when you want to add a "Falling" state next? What about a "Grounded Attack" and "InAir Attack" state? What about a "Hit" state? A "Dead" state?

The more states you add, the exponentially more conditions and transitions you have to add to each relevant state in the state machine. That's O(n^2) number of transitions in the worst case, and it'll affect your performance if you are checking every one of them every frame. This simply results in...a mess of unreadable stringed connections.
...
To further reduce complexity, you can also use **HSMModule**. For example, adding a "Gravity" module as a child to the "InAir" state will **not only** improve readability but will also improve performance when not in the air. Adding an "Animation Change" module as a child to your states will also prevent the need to constantly write redudant references to the AnimationPlayer in your code. Etc.
