...
This state machine can be separated into three main compoenents.
...
HSMNode nodes are the main node type that will hold any state functionality you want to be implemented. Physics frame, process frame, unhandled input, as well as cache changes are signaled within these nodes. You made handel these inputs as you wish when they are relevant.
...
The system is designed to reduce code redudancy.

### HSMModule

To further reduce redudancy, HSMModule are essentially the leaf node of any state machine. If a HSMNode node is active, then all of it's children HSMModule nodes will also be active.
...
These should be your primary method of state communicate, as well as facilitating when to change states.

## Efficency Measures

Each HSMNode and HSMModule node possess the method \_get_process_requirements(). Via overloading this, you can tell the state machine what methods this method need to be called at relevant times. Thsi drastically reduces overhead of having multiple unneeded functions be called, for each state, per frame.
